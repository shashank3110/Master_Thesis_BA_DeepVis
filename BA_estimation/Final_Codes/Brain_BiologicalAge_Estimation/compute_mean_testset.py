'''
Computing a mean image of the test set 
for each CDR and gender.
'''
...
def get_selected_scan_from_subjects(data_path,subject_ids,label_df,selected_scans):
    ...
    for subject in subject_ids :
       ...
       labels.extend([label_df[label_df['MRI ID']==scan_id.split('.')[0]]['Age'].to_list()[0] for scan_id in paths  \
        if scan_id.split('/')[-1].split('.')[0] in selected_scans ])
...
def get_test_files(label_path,data_path,debug_mode_subject=None,selected_scans=[]):
    ...
    if len(selected_scans)>0:
      test_patients,test_labels,test_gender,scan_ids,test_cdr = \
      get_selected_scan_from_subjects(data_path,test_ids,data,selected_scans)
    else:
...
data_path= ['/no_backups/g009/data/OASIS/tfrecords_data_exp4/testing_all_cdr',\
'/no_backups/g009/data/OASIS/tfrecords_data_exp4/training_cdr0']
test_patients,scan_ids, test_labels,test_gender,test_cdr = get_test_files(label_path,\
  data_path,debug_mode_subject=sub,selected_scans=scans)
...
for g in gender_cdr_full_dict.keys():
          ...
          for cdr,_ in gender_cdr_full_dict[g].items():
              ...
              savemat(matpath+'slice'+str(slice_id)+'axial_img_mean_cdr'+str(cdr)+'_'+gender_dict[g]+'.mat',\
                {'data': full_mean,'shape':full_mean.shape})
              ...
              print(slice_mean.shape)
              savemat(matpath+'slice'+str(slice_id)+'sagittal_img_mean_cdr'+str(cdr)+'_'+gender_dict[g]+'.mat',\
                {'data': full_mean,'shape':full_mean.shape})
              ...
              print(slice_mean.shape)
              savemat(matpath+'slice'+str(slice_id)+'coronal_img_mean_cdr'+str(cdr)+'_'+gender_dict[g]+'.mat',\
                {'data': full_mean,'shape':full_mean.shape})
