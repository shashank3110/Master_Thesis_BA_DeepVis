...
def __grouped_convolution_block(input, grouped_channels, cardinality, strides, weight_decay=5e-4):
    ...
    for c in range(cardinality):
        ...
        if c < (cardinality/2):
            ...
            #      kernel_initializer='he_normal', kernel_regularizer=l2(weight_decay))(init)
            x = Conv3D(filters=grouped_channels, kernel_size=(3,3,3),
                          strides=(strides, strides, strides), kernel_initializer='he_normal',
                          padding='same',
...
def __bottleneck_block(input, filters=64, cardinality=8, strides=1, weight_decay=5e-4):
    ...
    if init._keras_shape[-1] != filters:
            ...
            init = Conv3D(filters=filters, kernel_size=(3,3,3),
                          strides=(strides, strides, strides), kernel_initializer='he_normal',
                          padding='same',
                          kernel_regularizer=l2(weight_decay))(init)
            print(init)
...
def __create_res_next(weight_decay, depth, cardinality, width,
    ...
    x = MaxPooling3D((4, 4, 4), strides=(4, 4, 4), padding='same')(x)
    x = Flatten()(x)
    return x
...
def createModel(patchSize, weight_decay, depth, cardinality, width, num_dense_layers, num_dense_nodes):
...
if __name__ == '__main__':
    model,_ = createModel([121, 145, 121, 1], weight_decay=4e-5, depth=[1,1,1], cardinality = 4, width = 4, num_dense_layers = 4, num_dense_nodes=128 )
    plot_model(model, to_file='/home/d1308/no_backup/d1308/3T_Brain_age_regression/network//BioAge_Regression_Network_3D.png', show_shapes=True)
    model.summary()
