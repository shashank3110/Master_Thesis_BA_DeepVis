...
def __bottleneck_block(input, filters=64, cardinality=8, strides=1, weight_decay=5e-4):
    ...
    if init._keras_shape[-1] != filters:
            ...
            #Have replaced maxpooling by conv layer here
            init = Conv2D(filters, (1, 1), padding='same', strides=(strides, strides),
                          use_bias=False, kernel_initializer='he_normal', kernel_regularizer=l2(weight_decay))(init)
            init = BatchNormalization(axis=channel_axis)(init)
...
if __name__ == '__main__':
    model,_ = createModel([121,145,10], weight_decay=4e-5, depth=[2,2,2], cardinality = 8, width = 8)
    plot_model(model, to_file='/home/d1308/no_backup/d1308/3T_Brain_age_regression/network/BioAge_Regression_Network_2D.png', show_shapes=True)
    model.summary()
