...
def DenseNet(img_input=None, dense_blocks=1, dense_layers=-1, growth_rate=12, dropout_rate=None,
             bottleneck=True, compression=1.0, weight_decay=1e-4, depth=14):
    """
    Creating a DenseNet
...
def transition_layer(x, nb_channels, dropout_rate=None, compression=1.0, weight_decay=1e-4):
    ...
    x = BatchNormalization(gamma_regularizer=l2(weight_decay), beta_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    x = Conv3D(int(nb_channels*compression), (1, 1, 3), padding='same',
...
if __name__ == '__main__':
    model = DenseNet([121,145,121, 1])
    plot_model(model, to_file='/home/d1308/no_backup/d1308/3T_Brain_age_regression/network/dense.png', show_shapes=True)
    model.summary()