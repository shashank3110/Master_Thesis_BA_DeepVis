...
def tfdata_generator_volume_chunks(file_lists, label_lists, num_parallel_calls,train_patch_size, samples):
    ...
    dataset = dataset.map(
        map_func=lambda a, b: (convert_tf.parse_function_image(a), (tf.cast(b, tf.float32))),
